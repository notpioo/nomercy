...
// Create hybrid storage system - PostgreSQL first, Firestore fallback
class HybridStorage implements IStorage {
  ...
  async getUser(id: string): Promise<User | undefined> {
    ...
    let user = await this.pgStorage.getUser(id);
    if (user) return user;

    // Fallback to Firestore
    return await this.firestoreStorage.getUser(id);
  }

  async getUserByUsername(username: string): Promise<User | undefined> {
    let user = await this.pgStorage.getUserByUsername(username);
    if (user) return user;

    return await this.firestoreStorage.getUserByUsername(username);
  }

  async createUser(user: InsertUser): Promise<User> {
    try {
      // Try PostgreSQL first
      return await this.pgStorage.createUser(user);
    } catch (error) {
      console.error("PostgreSQL create user failed, using Firestore:", error);
      return await this.firestoreStorage.createUser(user);
    }
  }

  async getAllUsers(): Promise<User[]> {
    const pgUsers = await this.pgStorage.getAllUsers();
    if (pgUsers.length > 0) return pgUsers;

    return await this.firestoreStorage.getAllUsers();
  }

  async getUserStats(): Promise<{ totalUsers: number; activeUsers: number; totalGamesPlayed: number; totalRevenue: number }> {
    const pgStats = await this.pgStorage.getUserStats();
    if (pgStats.totalUsers > 0) return pgStats;

    return await this.firestoreStorage.getUserStats();
  }

  async updateUserCoins(id: string, mercyCoins: number, gems: number): Promise<void> {
    await this.pgStorage.updateUserCoins(id, mercyCoins, gems);
    await this.firestoreStorage.updateUserCoins(id, mercyCoins, gems);
  }

  async updateUserRank(id: string, rank: string, rankLevel: number): Promise<void> {
    await this.pgStorage.updateUserRank(id, rank, rankLevel);
    await this.firestoreStorage.updateUserRank(id, rank, rankLevel);
  }

  async updateUserGameStats(id: string, gamesPlayed: number, wins: number): Promise<void> {
    await this.pgStorage.updateUserGameStats(id, gamesPlayed, wins);
    await this.firestoreStorage.updateUserGameStats(id, gamesPlayed, wins);
  }

  // Delegate other methods to Firestore (for now)
  async getAllQuizzes(): Promise<any[]> {
    return await this.firestoreStorage.getAllQuizzes();
  }

  async initializeDefaultQuizzes(): Promise<void> {
    return await this.firestoreStorage.initializeDefaultQuizzes();
  }

  async createQuiz(quizData: any): Promise<any> {
    return await this.firestoreStorage.createQuiz(quizData);
  }

  async updateQuiz(quizId: string, updateData: any): Promise<void> {
    return await this.firestoreStorage.updateQuiz(quizId, updateData);
  }

  async deleteQuiz(quizId: string): Promise<void> {
    return await this.firestoreStorage.deleteQuiz(quizId);
  }

  async getAllRedeemCodes(): Promise<any[]> {
    return await this.firestoreStorage.getAllRedeemCodes();
  }

  async initializeDefaultRedeemCodes(): Promise<void> {
    return await this.firestoreStorage.initializeDefaultRedeemCodes();
  }

  async createRedeemCode(codeData: any): Promise<any> {
    return await this.firestoreStorage.createRedeemCode(codeData);
  }

  async updateRedeemCode(codeId: string, updateData: any): Promise<void> {
    return await this.firestoreStorage.updateRedeemCode(codeId, updateData);
  }

  async deleteRedeemCode(codeId: string): Promise<void> {
    return await this.firestoreStorage.deleteRedeemCode(codeId);
  }

  async getRedeemCodeByCode(code: string): Promise<any | null> {
    return await this.firestoreStorage.getRedeemCodeByCode(code);
  }

  async hasUserUsedRedeemCode(codeId: string, userId: string): Promise<boolean> {
    return await this.firestoreStorage.hasUserUsedRedeemCode(codeId, userId);
  }

  async markRedeemCodeAsUsedByUser(codeId: string, userId: string): Promise<void> {
    return await this.firestoreStorage.markRedeemCodeAsUsedByUser(codeId, userId);
  }

  async getRedeemCodeCurrentUses(codeId: string): Promise<number> {
    return await this.firestoreStorage.getRedeemCodeCurrentUses(codeId);
  }

  async getAllGameSettings(): Promise<any[]> {
    return await this.firestoreStorage.getAllGameSettings();
  }

  async initializeDefaultGameSettings(): Promise<void> {
    return await this.firestoreStorage.initializeDefaultGameSettings();
  }

  async updateGameSettings(gameType: string, settings: any): Promise<void> {
    return await this.firestoreStorage.updateGameSettings(gameType, settings);
  }

  async updateUser(id: string, updateData: Partial<User>): Promise<void> {
    return await this.firestoreStorage.updateUser(id, updateData);
  }

  async deleteUser(userId: string): Promise<void> {
    return await this.firestoreStorage.deleteUser(userId);
  }

  async createAdminUser(userId: string): Promise<User> {
    try {
      return await this.pgStorage.createAdminUser(userId);
    } catch (error) {
      console.error("PostgreSQL create admin failed, using Firestore:", error);
      return await this.firestoreStorage.createAdminUser(userId);
    }
  }
}
